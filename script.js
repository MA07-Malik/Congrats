...
Congratulations on achieving your amazing 3.8 GPA!

Every assignment, every exam, every sleepless night, and every challenge has led you to this incredible achievement.

You should be incredibly proud of yourself.

May you continue to heal hearts, inspire others, and become an amazing nurse.
...
function openEnvelope() {
  ...
  setTimeout(() => typeWriter(message, letterContent, 25), 1200);
}
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  if (surpriseButton) {
    surpriseButton.addEventListener('click', () => {
      ...
      if (envelopeWrap) envelopeWrap.classList.remove('hidden');
      setTimeout(() => {
        if (letter) letter.classList.add('open');
      }, 500);
      setTimeout(() => {
        typeWriter(message, letterContent, 25);
      }, 1200);
      triggerCelebration();
    });
